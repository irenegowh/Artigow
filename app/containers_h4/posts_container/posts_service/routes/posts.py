...
from posts_service.services.post_service import create_post, list_all_posts, delete_all_user_posts, get_post_by_id
import logging
from marshmallow import ValidationError
from posts_service.schemas.post_schema import PostSchema  # Ajustar el import para el esquema
...
def new_post():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            create_post(data['title'], data['content'], data['image'], current_user.username)  # Crear el post
            logger.info(f"Post creado exitosamente por {current_user.username}.")
...
def list_posts():
    ...
    try:
        posts = list_all_posts()  # Obtener todos los posts
...
def delete_all_posts():
    ...
    try:
        delete_all_user_posts(current_user.username)  # Borrar todas las publicaciones del usuario
        logger.info(f"Publicaciones de {current_user.username} eliminadas exitosamente.")
...
def show_post(post_id):
    ...
    try:
        post = get_post_by_id(post_id)  # Obtener el post por ID
        if post:
            return render_template('show_post.html', post=post)
        else:
            logger.error(f"Post con ID {post_id} no encontrado.")
            abort(404)
    except ValueError as e:
